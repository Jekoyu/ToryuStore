...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit') {
  ...
  // Cek jika gambar diupload, jika tidak ambil gambar yang ada
  if (!empty($_FILES['img']['tmp_name'])) {
    $img = file_get_contents($_FILES['img']['tmp_name']);
    ...
    // Ambil gambar yang ada di database untuk produk yang sama jika gambar baru tidak diupload
    ...
    $img = $row['img'];  // Menggunakan gambar yang ada
...
<body>
    ...
    <div class="modal-dialog">
      <form method="POST" enctype="multipart/form-data" class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3"><label>Harga</label><input type="number" name="harga_produk" class="form-control" required></div>
          <div class="mb-3"><label>Stok</label><input type="number" name="stock" class="form-control" required></div>
          <div class="mb-3"><label>Gambar</label><input type="file" name="img" class="form-control"></div>
        </div>
    ...
    <div class="modal-dialog">
      <form method="POST" enctype="multipart/form-data" class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3"><label>Nama Produk</label><input type="text" name="nama_produk" id="editNamaProduk" class="form-control" required></div>
          <div class="mb-3"><label>Harga</label><input type="number" name="harga_produk" id="editHargaProduk" class="form-control" required></div>
          <div class="mb-3"><label>Stok</label><input type="number" name="stock" id="editStock" class="form-control" required></div>
          <div class="mb-3"><label>Gambar</label><input type="file" name="img" id="editImg" class="form-control"></div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
          <button type="submit" class="btn btn-primary">Simpan</button>
  ...
  <script>
    function fillEditForm(product) {
      console.log(product); // Debug: Lihat data produk di console
